...
from tseval.metnam.eval.MNModelLoader import MNModelLoader
...
class StandardSetup(EvalSetupBase):
    ...
    def prepare(self) -> None:
        ...
        dataset: List[MethodData] = MethodData.load_dataset(
            ...
            only=["code", "code_masked", "name", "misc"],
            tbar=tbar,
        )
        tbar.close()
        ...
        name_list = []
        for d in dataset:
            # Replace the mask token such that the code remains parsable
            code_masked = d.code_masked.replace("<MASK>", "METHODNAMEMASK")
            d.fill_none()
            d.misc["orig_code"] = d.code
            d.misc["orig_code_masked"] = code_masked
            d.misc["orig_name"] = d.name
            orig_code_list.append(code_masked)
        ...
        tbar.set_description("Subtokenizing")
        tbar.reset(len(dataset))
        for d, tokenized_code, name in zip(dataset, tokenized_code_list, name_list):
            d.code, d.misc["code_src_ids"] = ModelUtils.subtokenize_batch(tokenized_code)
            d.code_masked = None
            d.name = ModelUtils.subtokenize(name)

            # convert name to lower case
            d.name = [t.lower() for t in d.name]
            tbar.update(1)
        tbar.close()
    ...
    def clean_eval_set(self, indexed_dataset: Dict[int, MethodData], eval_ids: List[int]) -> List[int]:
        ...
        Keeps the eval set absolutely clean by:
          - Remove duplicate (name, code) pairs;
        ...
        for i in eval_ids:
            ...
            # Remove duplicate (name, code) pairs
            data_key = (tuple(data.code), tuple(data.name))
    ...
    def train(self, exp_name: str, model_name: str, cont_train: bool, no_save: bool, **options) -> None:
        ...
        model = MNModelLoader.init_or_load_model(model_name, exp_dir, cont_train, no_save, options)
        if not no_save:
    ...
    def eval_one(self, exp_name: str, eval_ids: List[int], prefix: str, indexed_dataset: Dict[int, MethodData], model: MNModelBase, gpu_id: int = 0):
    ...
    def eval(self, exp_name: str, action: str = None, gpu_id: int = 0) -> None:
        ...
        # Load model
        exp_dir = self.get_exp_dir(exp_name)
        model: MNModelBase = MNModelLoader.load_model(exp_dir)
