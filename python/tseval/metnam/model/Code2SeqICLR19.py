...
class Code2SeqICLR19Config(RecordClass):
    seed: int = None
...
class Code2SeqICLR19(MNModelBase):
    ENV_NAME = "tseval-MN-Code2SeqICLR19"
    SRC_DIR = Macros.work_dir / "src" / "MN-Code2SeqICLR19"
    ...
    def prepare_train_val_data(
        ...
        s += f"python preprocess.py --train_data {train_data_file} --val_data {val_data_file}" \
             ...
             f" --max_data_contexts {self.config.max_data_contexts}" \
             f" --subtoken_vocab_size {self.config.subtoken_vocab_size}" \
             f" --target_vocab_size {self.config.target_vocab_size}" \
             f" --subtoken_histogram {subtok_hist_file}" \
             f" --node_histogram {node_hist_file}" \
